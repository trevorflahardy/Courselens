...
async def export_markdown() -> str:
    ...
    if not changes:
        out.write("> No changes have been recorded yet.\n")
        return out.getvalue()
    ...
    # --- Per-assignment change sections ---
    grouped: dict[str, list[AppliedChange]] = defaultdict(list)
    for c in changes:
        grouped[c.node_id].append(c)

    # Assignments sorted by week then title; changes within an assignment
    # sorted oldest-first so the boss reads a chronological story.
    ...
    for node_id in sorted(grouped, key=_assignment_sort_key):
        meta = assignments.get(node_id, {})
        title = meta.get("title") or node_id
        week = meta.get("week")
        week_label = f"Week {week} — " if week is not None else ""
        count = len(grouped[node_id])
        out.write(f"## {week_label}Assignment: \"{title}\"\n")
        out.write(f"_Node id: `{node_id}` · {count} change{'s' if count != 1 else ''}_\n\n")

        for idx, c in enumerate(
            sorted(grouped[node_id], key=lambda x: x.created_at),
            start=1,
        ):
